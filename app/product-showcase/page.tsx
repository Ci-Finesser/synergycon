...
import { ArrowLeft, Check, Grid3x3, List, Sparkles, ArrowRight, Info } from "lucide-react"
import Link from "next/link"
import { createClient } from "@/lib/supabase/client"
import { BioModal } from "@/components/bio-modal"
import { PartnerCard } from "@/components/partner-card"
import { PRODUCT_SHOWCASE_TIERS } from "@/lib/partnership-tiers"
...
export default function ProductShowcasePage() {
  ...
  const [selectedProduct, setSelectedProduct] = useState<Partner | null>(null)
  ...
  const openProductModal = (product: Partner) => {
    ...
    <div className="bg-neutral-50 border-2 border-neutral-200 rounded-xl p-5 md:p-6">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          <h3 className="text-sm md:text-base font-semibold mb-2.5 md:mb-3">No Products Yet</h3>
          <p className="text-xs md:text-sm text-muted-foreground leading-relaxed mb-4">
            We're currently accepting product showcase applications. Have an innovative product to display?
          </p>
          <Link href="/become-partner?type=product-showcase">
  ...
  const renderProductCards = () => (
    ...
    >
      {products.map((product) => (
        <PartnerCard key={product.id} partner={product} viewMode={viewMode} onClick={openProductModal} />
      ))}
    </div>
  )
    ...
    <main className="min-h-screen">
          ...
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 md:gap-6 mb-5 md:mb-6">
            <div className="max-w-3xl">
              ...
              <p className="text-sm md:text-base text-muted-foreground text-pretty leading-relaxed">
                Discover innovative products from Nigeria's creative and tech ecosystem at SynergyCon 2.0.
              </p>
            </div>
              ...
              {PRODUCT_SHOWCASE_TIERS.map((tier, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <Link href={`/become-partner?type=product-showcase&tier=${tierValue}`} className="w-full">
                      <Button
                        variant="default"
      ...
      {selectedProduct && (
        <BioModal
          ...
          bio={selectedProduct.bio || "No bio available"}
          image={selectedProduct.logo_url}
